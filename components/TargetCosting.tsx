            ...
            <div className="p-6 max-w-4xl mx-auto">
                <div className="space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 text-center">
                        <label htmlFor="overall-target" className="text-lg font-semibold text-gray-800">
                            What's your target food cost %?
                        </label>
                        <p className="my-4 text-6xl font-bold text-blue-600 tracking-tight">{overallTarget}%</p>
                        <input
                            id="overall-target"
                            type="range"
                            min="15"
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                        <h3 className="text-lg font-semibold text-gray-800">Or set different targets by category:</h3>
                        <p className="text-sm text-gray-500 mt-1 mb-6">This is optional and overrides the overall target for specific recipes.</p>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {Object.entries(categoryTargets).map(([category, value]) => (
                                <div key={category}>
                                    <label htmlFor={`cat-${category}`} className="block text-sm font-medium text-gray-700">{category}</label>
                                    <div className="relative mt-1">
                                        <input
                                            ...
                                            className="w-full pl-3 pr-8 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                        />
                    ...
                    <div className="flex justify-end items-center">
                        {saved && <p className="text-sm font-medium text-green-600 mr-4">Changes saved successfully!</p>}
                        <button
                            onClick={handleSave}
                            className="px-6 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                        >
                            Save Changes
                        </button>
                    </div>
